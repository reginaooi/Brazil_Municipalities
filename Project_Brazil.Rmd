...
The data used was gathered by Professor Kam Tin Seong for academic purposes.

```{r echo=TRUE, eval=FALSE, warning=FALSE, message=FALSE}
packages = c('olsrr', 'corrplot', 'ggpubr', 'sf', 'spdep', 'GWmodel', 'tmap', 'tidyverse', 'geobr', 'fastDummies', 'EnvStats','kableExtra')
for (p in packages){
...
brazilcities <- read_csv2("data/aspatial/BRAZIL_CITIES.csv")
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
datadict <- read_csv2("data/aspatial/Data_Dictionary.csv")
...
### Extracting the selected columns from brazilcities
...
brazilcities_GDP <- brazilcities %>%
 dplyr::select("CITY", "STATE", "CAPITAL", "LONG", "LAT", "GDP_CAPITA", "IBGE_15-59", "IDHM_Educacao", "IDHM_Longevidade", "IDHM_Renda", "AREA", "TAXES", "COMP_TOT", "RURAL_URBAN", "IBGE_RES_POP", "IBGE_POP", "IBGE_DU_URBAN", "IBGE_DU_RURAL", "IBGE_DU", "IBGE_PLANTED_AREA", "GVA_AGROPEC", "GVA_INDUSTRY", "GVA_SERVICES", "GVA_PUBLIC", "POP_GDP", "Cars", "Motorcycles", "PAY_TV", "FIXED_PHONES", "IBGE_RES_POP_BRAS") %>%
  ...
  mutate(`working_prop` = `IBGE_15-59`/`IBGE_POP`) %>%
...
Looking at specific rows and columns with NA values

```{r echo=FALSE, warning=FALSE, message=FALSE}
brazilcities_GDP[rowSums(is.na(brazilcities_GDP))!=0,]
```
...
## Interactive Map of GDP per capita
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(data = brazilcities.sf,  
       aes(x = industry_prop, y = gdp_capita)) +
  geom_point() +
  geom_smooth(method = lm)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(data = brazilcities.sf,  
       aes(x = services_prop, y = gdp_capita)) +
  geom_point() +
  geom_smooth(method = lm)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(data = brazilcities.sf,  
       aes(x = public_prop, y = gdp_capita)) +
  geom_point() +
  geom_smooth(method = lm)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE}
ggplot(data = brazilcities.sf,  
       aes(x = agropec_prop, y = gdp_capita)) +
  geom_point() +
  geom_smooth(method = lm)
```
...
### Multiple linear regression model without transformation
...
### Multi-linear regression model (with transformation)
...
### Revised Multiple Linear Regression Model
...
# Explanatory model using Geographically Weighted Regression
...
Coefficient Standard Error measure the reliability of each coefficient estimate. We want the coefficients of standard errors to be small. Large coefficients may mean that there are problems with local collinearity.
